...
class HTMLExtractor:
    ...
    def __extract_detail_web(self, data: Namespace, id_: str) -> dict:
        ...
        manifest = Namespace.object_extract(data, f"{detail}.manifestH265.json.adaptationSet")
        if not manifest or len(manifest) == 0:
            return container
        repre = Namespace.object_extract(manifest[0], "representation")
        if not repre or len(repre) == 0:
            return container
        container['size'] = Namespace.object_extract(repre[0], "fileSize", 0)
        width = Namespace.object_extract(repre[0], "width", 0)
        height = Namespace.object_extract(repre[0], "height", 0)
        container['resolution'] = f"{width}x{height}"
        return container
